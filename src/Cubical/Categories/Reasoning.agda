...
step-cong-≡ : ∀ {a b c d b' c'} (x : Expr a b c d) {y : C' [ b , c ]} {z : Expr a b' c' d} → (eval (replaceFocus x y)) IsRelatedTo z → focus x ≡ y → eval x IsRelatedTo z
step-cong-≡ x (relTo p) q = relTo (congExpr x q ∙ p)

step-cong-≡˘ : ∀ {a b c d b' c'} (x : Expr a b c d) {y : C' [ b , c ]} {z : Expr a b' c' d} → (eval (replaceFocus x y)) IsRelatedTo z → y ≡ focus x → eval x IsRelatedTo z
